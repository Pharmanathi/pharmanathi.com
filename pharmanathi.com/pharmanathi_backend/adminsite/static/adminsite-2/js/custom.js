...
function markMHPAsVerified(mhpId) {
    fetch(`/custom-admin/MHPs/actions/${mhpId}/mark-verified/`, { headers: GLOBAL_CONF.defaultHTTPRequestHeader() })
        .then(res => {
...
function invalidateMHPProfile(mhpId) {
    fetch(`/custom-admin/MHPs/actions/${mhpId}/invalidate/`, { headers: GLOBAL_CONF.defaultHTTPRequestHeader() })
        .then(res => {
            if (res.ok) location.reload()
            else return res.json()
        })
...
document.querySelector("#btn-mhp-invalidate-mhp-profile").addEventListener("click", (e)=>{
    invalidateMHPProfile(e.target.dataset.mhpId)
})